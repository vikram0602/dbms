...
$temp_agg_b="SELECT avg(temperature) as tmp,to_char(time_stamp,'MM') as dte from climate_data where station_id=:station_id and time_stamp between TO_DATE ('$yf/$mf/01', 'yyyy/mm/dd') AND TO_DATE ('$yt/$mt/28', 'yyyy/mm/dd') group by TO_CHAR(time_stamp,'MM') order by dte asc";
